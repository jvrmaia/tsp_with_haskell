...
kdtBuild [pt0,pt1]-- como default p/ 2 nums sempre por y!!!
	|ycoord pt0 <= ycoord pt1 = Node pt0 ycoord (Leaf pt0) (Leaf pt1)
	|otherwise = Node pt1 ycoord (Leaf pt1) (Leaf pt0)
kdtBuild (p0:p1:p2:pts) = Node md ld (kdtBuild $ mi:md:[x|x<-(pts),ld x<=ld md]) (kdtBuild $ ma:[x|x<-(pts),ld x>ld md])
	where
    		ld
...
kdtDel :: Point -> KDTree -> KDTree
kdtDel pt tree =
	case reckdtDel pt tree of
		OK t -> t
		NOK -> tree

reckdtDel :: Point -> KDTree -> DelOK
reckdtDel (pt) (Leaf lpt)
	| pt == lpt = OK (DeadLeaf pt)
	| otherwise = NOK
reckdtDel (pt) (DeadLeaf lpt) = NOK
reckdtDel pt a@(DeadNode median dim lnode rnode) = NOK
reckdtDel pt a@(Node median dim lnode rnode)
	| dim pt < dim median = 
		case reckdtDel pt lnode of
